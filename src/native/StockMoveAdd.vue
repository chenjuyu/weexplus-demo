        ...
        <list :style="{height: pageheight+'px'}">
            <cell >
                ...
                <wxc-cell label="转出类别:"
                          ...
                          :has-bottom-border="true"	>
                    <text style="direction: rtl;margin-right: 10px;font-size: 30px;color: #666666;text-align: right">{{MoveOutType}}</text>
                </wxc-cell>
                ...
                <wxc-cell label="转进仓库:"
                          ...
                          :has-bottom-border="true"	>
                    <text style="direction: rtl;margin-right: 10px;font-size: 30px;color: #666666;text-align: right">{{WarehouseIn.WarehouseIn}}</text>
                </wxc-cell>
...
<script>
    ...
    var url='/stockMove.do?stockMoveEditX'
    var saveurl='/stockMove.do?saveStockMoveX'
    ...
    export default {
        ...
        ,methods:{
            onLoad(p){
                ...
                this.StockMoveID=p.hasOwnProperty("StockMoveID")?p.StockMoveID:''   //|| 'DN000XW'//''
                // this.direction= p.hasOwnProperty("direction")?p.direction:1
                //  this.title = p.hasOwnProperty("title")?p.title:''
                this.log('StockMoveID的值：'+this.StockMoveID)


                if(this.StockMoveID ==''){ //可能是新增单据，也有可能是从其他页面点的返回键
                    return
                }
                ...
                this.Department.DepartmentID =p.hasOwnProperty("DepartmentID")?p.DepartmentID:pstatic.getString('DepartmentID')
                this.Department.Department =p.hasOwnProperty("Department")?p.Department:pstatic.getString('Department')
                ...
                this.WarehouseIn.WarehouseInID =p.hasOwnProperty("WarehouseInID")?p.WarehouseInID:'' //代表转进仓
                this.WarehouseIn.WarehouseIn =p.hasOwnProperty("WarehouseIn")?p.WarehouseIn:''
                ...
                net.post(pref.getString('ip') + url,{StockMoveID:this.StockMoveID,direction:this.direction},{},function(){
                    //start
                ...
                nav.pushFull({url:'root:base.js',param: p,
                    ...
                    if (res !=undefined) {
                        if (id==1){ //发货部门
                            ...
                            this.WarehouseIn.WarehouseInID=res.id
                            this.WarehouseIn.WarehouseIn =res.Name
                            this.customer.customerid =res.SettleCustID
            ...
            ,selectType(e){
                ...
                for(var i=0 ;i< this.list.length ;i++){
                    var map=this.list[i]
                    if(this.billType.Name == map.value && e==1){
                        map.checked=true
                    }else if(this.MoveOutType ==map.value && e==2){
                        map.checked=true
                    }
                }
                this.show=true
            ...
            , wxcRadioListChecked (e) {
                ...
                if(this.show) {
                    ...
                    this.MoveOutType =e.value
                    if (e.value == '批发') {
                        this.MoveOutTypeID  = 'PriceType'
                    } else if (e.value == '订货') {
                        this.MoveOutTypeID  = 'OrderPriceType'
                    } else if (e.value == '配货') {
                        this.MoveOutTypeID  = 'AllotPriceType'
                    } else if (e.value == '补货') {
                        this.MoveOutTypeID  = 'ReplenishType'
                    }
                    this.inshow=false
                }
                ...
                if (this.mobileX < 0) {
                    ...
                    if(node.sizeData.length>0) {
                        var obj =(this.detaillist.filter(item=>item.GoodsID ==node.GoodsID)).map(function (obj) {  // node.sizeData.map(function (obj) {
                            return {
                                StockMoveDetailID:obj.hasOwnProperty("StockMoveDetailID")?obj.StockMoveDetailID:'',
                                StockMoveID:obj.hasOwnProperty("StockMoveID")?obj.StockMoveID:'',
                                GoodsID: obj.GoodsID,
                                Code:obj.Code,
                                img:obj.img,
                                Name:obj.Name,
                        ...
                        for (var i = 0; i < obj.length; i++) {
                            debugger
                            var map = {}
                            map.StockMoveDetailID=obj[i].StockMoveDetailID
                            map.StockMoveID=obj[i].StockMoveID
            ...
            save(){ //保存单据
                ...
                if(this.WarehouseIn.WarehouseInID ==''){
                    that.toast('请选择转进仓库')
                    return;
                }
                ...
                var p={},param={}
                p.StockMoveID =this.StockMoveID
                ...
                p.WarehouseInID=this.WarehouseIn.WarehouseInID
                p.WarehouseIn =this.WarehouseIn.WarehouseIn
                p.WarehouseOutID=this.Department.DepartmentID
                p.WarehouseOut=this.Department.Department
                p.employeeId =this.emp.EmpID //经手人
                p.brandId ='' //品牌
                p.businessDeptId ='' //业务部门
                p.paymentTypeId=this.ReceivalType[0].PaymentTypeID//结算方式
                p.memo ='手机APP生成' //备注
                p.MoveInType =this.billType.Name //单据类别
                p.MoveOutType =this.MoveOutType
                ...
                net.post(pref.getString('ip') + saveurl,p,{},function(){
                    ...
                    //success
                    progress.dismiss()
                    that.StockMoveID =e.res.obj
                    that.toast('保存成功')
                    param.direction =p.direction
                    param.title=that.title
                    nav.pushParam('root:stockmovelist.js',param)
                    //var p={}
            ...
            ,addgoods(e){ //如果有就是修改
                ...
                if(this.Department.DepartmentID ==this.WarehouseIn.WarehouseInID){
                    this.toast('请先选择转进仓库与转出仓库不能相同')
                    return
