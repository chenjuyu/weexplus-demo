<template>
    <div class="wrapper">
        ...
        <div class="search" >
            ...
            <div style="height: 70px;width: 220px;margin-right: 1px;border-width: 1px;justify-content: center;align-items: center;border-color: #D3D3D3;"  @click="search">
...
<script>
    ...
    var url='/purchaseorder.do?purchaseOrderEditX'
    var saveurl='/purchaseorder.do?savePurchaseOrderX'
    ...
    export default {
        ...
        ,methods:{
            onLoad(p){
                ...
                this.PurchaseOrderID=p.hasOwnProperty("PurchaseOrderID")?p.PurchaseOrderID:''   //|| 'DN000XW'//''
                this.direction =p.hasOwnProperty("direction")?p.direction:1
                this.title =p.hasOwnProperty('title')?p.title:''
                this.log('PurchaseOrderID:'+this.PurchaseOrderID)




                if(this.PurchaseOrderID ==''){ //可能是新增单据，也有可能是从其他页面点的返回键
                    return
                }
               ...
               // this.title =p.hasOwnProperty("title")?p.title:''
                ...
                net.post(pref.getString('ip') + url,{PurchaseOrderID:this.PurchaseOrderID,direction:this.direction},{},function(){
                    //start
                ...
                if (this.mobileX < 0) {
                    ...
                    if(node.sizeData.length>0) {
                        var obj =(this.detaillist.filter(item=>item.GoodsID ==node.GoodsID)).map(function (obj) {  // node.sizeData.map(function (obj) {
                            return {
                                PurchaseOrderDetailID:obj.hasOwnProperty("PurchaseOrderDetailID")?obj.PurchaseOrderDetailID:'',
                                PurchaseOrderID:obj.hasOwnProperty("PurchaseOrderID")?obj.PurchaseOrderID:'',
                                GoodsID: obj.GoodsID,
                                Code:obj.Code,
                                img:obj.img,
                                Name:obj.Name,
                        ...
                        for (var i = 0; i < obj.length; i++) {
                            debugger
                            var map = {}
                            map.PurchaseOrderDetailID=obj[i].PurchaseOrderDetailID
                            map.PurchaseOrderID=obj[i].PurchaseOrderID
            ...
            save(){ //保存单据
                ...
                net.post(pref.getString('ip') + saveurl,p,{},function(){
                    ...
                    //success
                    progress.dismiss()
                    that.PurchaseOrderID =e.res.attributes.PurchaseOrderID
                    ...
                    if(e.res.attributes.tempList.length >0){
                        ...
                        that.toast('保存成功')
                        that.push('root:purchaseorderlist.js', {direction: that.direction, title: that.title})
                    }
                    //var p={}
