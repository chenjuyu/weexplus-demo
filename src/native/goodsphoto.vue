<template>
    <div class="wrapper">
        <head :rightText="rightText" title="货品拍照"  @rightClick="rightClick"></head>
        <div class="search"> <!-- style="width: 500px;height: 80px;border-width: 5px;border-color: #00B4FF;margin-left: 10px"-->
            ...
            <div style="border-width: 1px;border-color: #ccc;height: 70px;border-radius: 3px;width: 200px;align-items:center;justify-content: center"><text @click="add" style="font-size: 35px;">查询</text> </div>
        </div>
        <list style="flex: 1;">
            ...
            <cell ref="skid" v-for="(item, i) of data" @click="onNodeClick(item, i)" :key="'skid-' + i" class="wxc-skid" :style="{width: (750 + item.right.length * 100) + 'px', height: height + 'px'}" @touchstart="(e) => !isAndroid && onPanStart(e, item, i)" @horizontalpan="(e) => isAndroid && onPanStart(e, item, i)" @touchend="(e) => onPanEnd(e, item, i)">
                <div :style='styles' class="swipe-action-center border">
                    ...
                    <div style="justify-content: center;align-items: center;height: 200px"><text style="font-size: 35px;color:red">{{Number(i)+1}}</text></div>
                    <!-- 图片显示 -->
                    <div style="justify-content: center;align-items: center;height: 200px">
                    <image style="width: 150px;height: 150px;" :src="item.img"></image>
                    </div>
                    <div class="left">
                        <text style="font-size: 35px;height: 50px;font-weight:bold">{{item.Code}}</text>
                        <text style="font-size: 35px;height: 50px">{{item.Name}}</text>
                        <text style="font-size: 35px;height: 50px">季节:{{item.Season}}</text>
                        <text style="font-size: 35px;height: 50px">厂商:{{item.Supplier}}</text>
 ...
 <script>
     ...
     const plus=weex.requireModule('photoplus')

     const url='/goodsInfo.do?glist'
     const uploadurl='/common.do?uploadImages'
     export default {
         components: {  },
         props: {
             data: {//列表
 ...
 //                    net.postFile('https://b.kuaidimao.com/xapi/ocr_tel',param,{},path,()=>{
                         net.postFile(pref.getString('ip')+uploadurl+'&SalesID='+pNode.Code,param,{},path,()=>{
                             //start
 //                        process.show()
                             ...
                             if(e.res.success){
                                // pNode.img =e.res.obj
                                 Vue.set(that.data[i],'img',e.res.obj)
                ...
                // var qtysum=0,amtsum=0
                 ...
                 if (this.mobileX < 0) {
                     ...
                     //type:   video,photo
                     //action:  camera 相机 ；choose:相册
                     plus.open({type:'photo','action':'camera'},(res)=>{
 //                    this.video=res.res[0].path
 //                    this.src=res.res[0].path
                         this.src=res.res[0].path
                         var param={};
                         ...
                         path.file=res.res[0].path
 //                    net.postFile('https://b.kuaidimao.com/xapi/ocr_tel',param,{},path,()=>{
                         net.postFile(pref.getString('ip')+uploadurl+'&SalesID='+node.Code,param,{},path,()=>{
                             //start
 //                        process.show()
                             progress.showFull('正在上传',false)
                         },(e)=>{
