<template>
    <div class="container">
            ...
            <cell ref="skid" v-for="(item, i) of data" @click="onNodeClick(item, i)" :key="'skid-' + i" class="wxc-skid" :style="{width: (750 + item.right.length * 100) + 'px', height: height + 'px'}" @touchstart="(e) => !isAndroid && onPanStart(e, item, i)" @horizontalpan="(e) => isAndroid && onPanStart(e, item, i)" @touchend="(e) => onPanEnd(e, item, i)">
                <div :style='styles' class="swipe-action-center border">
                    ...
                    <div class="left">
                        ...
                        <text style="font-size: 35px;height: 60px" v-if="direction ==1">厂商:{{item.Supplier}}</text>
                        <text style="font-size: 35px;height: 60px" v-if="direction ==-1">客户:{{item.Customer}}</text>
                        <text style="font-size: 35px;height: 60px">类别:{{item.Type}}</text>
                    ...
                    <div class="right" style="position: absolute;right: 0">
                        ...
                        <text style="font-size: 35px;height: 60px">仓库名称:{{item.Department}}</text>
                        <text style="font-size: 35px;height: 60px">制单人:{{item.MadeBy}}</text>
                        <text style="font-size: 35px;height: 60px">结算余额:{{item.RelationAmountSum}}</text>
        ...
        <div class="footer">
            ...
            <div style="height: 80px;justify-content: center;align-items: center;position: absolute;right: 0;bottom: 0"> <text style="font-size: 40px;color: #FFFFFF;">结算金额:￥{{totalAmt}}</text></div>
        </div>
    </div>
</template>
...
<script>
    ...
    var url='/stock.do?stocklist'
    var auditurl ='/stock.do?auditOrder'
    export default {
        components: {
        },
        props: {
        ...
        methods: {
            onLoad(p){
                ...
                pstatic.set('stockmaster',param)//用于加载更多使用
                ...
                for(var i=0;i<this.data.length;i++){
                    ...
                    a=a+Number(this.data[i].RelationAmountSum) //改为统计结算金额
                }
                this.totalQty =q
                this.totalAmt =a
            ...
            ,add(e){
                nav.pushParam('root:StockAdd.js',{title:this.title,direction:this.direction})
            },search(){
                var that=this
                var param={}
                param.currPage=this.currPage
