        ...
        <list :style="{height: pageheight+'px'}">
            <cell >
                ...
                <wxc-cell label="客户名称:"
                          ...
                          :has-bottom-border="true"	>
                    <text style="direction: rtl;margin-right: 10px;font-size: 30px;color: #666666;text-align: right">{{customer.customerid}}</text>
                </wxc-cell>
                ...
                <div :style='styles' class="swipe-action-center border">
                    ...
                    <div style="flex-direction: row;width: 750px">
                        ...
                        <div style="position: absolute;right: 10px;">
                            <text style="font-size: 30px;color:#000000;height: 50px;">结算价:{{ls.RelationUnitPrice}}</text>
                            <text style="font-size: 30px;color:#000000;height: 50px;">折扣:{{ls.DiscountRate}}</text>
                            <text style="font-size: 30px;color:#000000;height: 50px;">结算金额:{{ls.RelationAmount}}</text>
                        </div>
                    </div>
                    <!-- 图片与显示-->
                    <div style="flex-direction: row">
                        <text style="font-size: 35px;color:red;margin-left: 5px;width: 60px;text-align: center" v-for="(lst,index1) in ls.sizetitle">{{lst.title}}</text>
                    </div>
                    <div style="flex-direction: row">
                        <text style="font-size: 35px;color:#000000;margin-left: 5px;width: 60px;text-align: center" v-for="(lst2,index2) in ls.sizeData">{{lst2.Quantity }}</text>
                    </div>
...
<script>
    ...
    var url='/stock.do?stockEditX'
    var saveurl='/stock.do?saveStockX'
    ...
    export default {
        ...
        ,methods:{
            onLoad(p){
                ...
                this.StockID=p.hasOwnProperty("StockID")?p.StockID:''   //|| 'DN000XW'//''
                this.direction =p.hasOwnProperty("direction")?p.direction:1
                this.title =p.hasOwnProperty('title')?p.title:''
                this.log('StockID:'+this.StockID)

                if(this.direction ==-1) {

                    this.totalqtytype ='数量合计:'
                    this.totalamttype='金额合计:'
                    this.list=[
                        { title: '销售', value: '销售' },
                        { title: '销售退货', value: '销售退货' },
                        { title: '转仓出仓', value: '转仓出仓' },//checked: true
                        { title: '盘亏', value:'盘亏' },
                        { title: '调整', value: '调整' },
                        { title: '赠品', value: '赠品' },
                        { title: '领用', value: '领用' },
                        { title: '差异', value: '差异' },
                        { title: '其它出仓', value: '其它出仓' },
                        { title: '返修', value: '返修' },
                        { title: '报损', value: '报损' }
                    ]
                    ...
                    this.list=[
                        { title: '采购', value: '采购' },
                        { title: '采购退货', value: '采购退货' },
                        { title: '转仓进仓', value: '转仓进仓' },//checked: true
                        { title: '盘盈', value:'盘盈' },
                        { title: '生产', value: '生产' },
                        { title: '生产退货', value: '生产退货' },
                        { title: '调整', value: '调整' },
                        { title: '差异', value: '差异' },
                        { title: '其它进仓', value: '其它进仓' },
                        { title: '返修', value: '返修' },
                        { title: '报溢', value: '报溢' }
                    ]
                }
                ...
                this.billType.id=''
                if(this.direction ==1) {
                    this.billType.Name = p.hasOwnProperty('Type') ? p.Type :'其它进仓'
                }else{
                    this.billType.Name = p.hasOwnProperty('Type')? p.Type :'其它出仓'
                }

                if(this.StockID ==''){ //可能是新增单据，也有可能是从其他页面点的返回键
                    return
                }
                this.log('StockID:'+this.StockID)
                ...
                net.post(pref.getString('ip') + url,{StockID:this.StockID,direction:this.direction},{},function(){
                    //start
            ...
            ,total(){
                ...
                for(var i=0 ;i<this.detaillist.length;i++){
                    ...
                    sumamt =Number(sumamt)+Number(this.detaillist[i].RelationAmount)
                ...
                if (this.mobileX < 0) {
                    ...
                    if(node.sizeData.length>0) {
                        var obj =(this.detaillist.filter(item=>item.GoodsID ==node.GoodsID)).map(function (obj) {  // node.sizeData.map(function (obj) {
                            return {
                                StockDetailID:obj.hasOwnProperty("StockDetailID")?obj.StockDetailID:'',
                                StockID:obj.hasOwnProperty("StockID")?obj.StockID:'',
                                GoodsID: obj.GoodsID,
                                Code:obj.Code,
                                img:obj.img,
                                Name:obj.Name,
                        ...
                        for (var i = 0; i < obj.length; i++) {
                            debugger
                            var map = {}
                            map.StockDetailID=obj[i].StockDetailID
                            map.StockID=obj[i].StockID
            ...
            save(){ //保存单据
             ...
             /*   if(this.Supplier.SupplierID ==''){
                ...
                net.post(pref.getString('ip') + saveurl,p,{},function(){
                    ...
                    //success
                    progress.dismiss()
                    that.StockID =e.res.attributes.StockID
                    ...
                    if(e.res.attributes.tempList.length >0){
                        ...
                        that.toast('保存成功')
                        that.push('root:stocklist.js', {direction: that.direction, title: that.title})
                    }
                    //var p={}
            ...
            ,hasmap(arr,map,isback){ //有一个就返回，最后判断，没有返回undefined,isback代表从其他页面返回的结果
                ...
                for (var j = 0; j < arr.length; j++) {
                    if (arr[j].GoodsID == map.GoodsID && arr[j].ColorID == map.ColorID && isback==0 ) {
                        ...
                        arr[j].RelationAmount =Number(map.RelationAmount)
                ...
                for (var j = 0; j < arr.length; j++) {
                    if (arr[j].GoodsID == map.GoodsID && arr[j].ColorID == map.ColorID && arr[j].SizeID ==map.SizeID && flag==0) {
                        ...
                        arr[j].RelationAmount =map.RelationAmount   //因为详情页没有把这个字段提交过去，所以只能要返回的时，重新增加这个字段
                        m  = arr[j]
            ...
            //左滑方法结束
            ,addgoods(e){ //如果有就是修改  如果 两个id为空，默认取货品资料的零售价
             /*   if(this.direction ==1) {
                    if (this.Supplier.SupplierID == '') {
                        this.toast('请先选择厂商')
                        return
                ...
                nav.pushFull({url: 'root:goodslistforstock.js',param:this.submitmap,animate:true}
                    ,(e)=>{
                        ...
                        if(e.detaillist.length >0){
                            this.log("返回的列表："+JSON.stringify(e.detaillist))
            ...
            ,search(){
                ...
                if(this.Supplier.SupplierID ==''){
                    ...
                    wherestr =this.Supplier.SupplierID
                }else{
                    if(this.customer.customerid ==''){
                        that.alert('请选择客户')
                        return;
                    }
                    wherestr =this.customer.customerid
                }
                if(this.AuditFlag){
                    this.toast('单据已审核无法再修改')
                    return;
                ...
                qr.open(p,(res)=>{
                    ...
                    // that.alert('扫到内容'+res) that.Supplier.SupplierID 改为 wherestr
                    net.post(pref.getString('ip')+qrcodeurl,{BarCode:res,CustomerId:wherestr,Type:that.billType.id},{},function(){
                        //start
                    },function(e){
                        //success  e.res;
